...
if (!class_exists('GNA_ContactForm7SMS_Admin_Menu')) {
	abstract class GNA_ContactForm7SMS_Admin_Menu {
		/**
		...
		function show_msg_settings_updated() {
			...
			_e('Settings successfully updated.', 'gna-contact-form-7-sms');
			echo '</strong></p></div>';
		}

		static function show_msg_record_deleted_st() {
			echo '<div id="message" class="updated fade"><p><strong>';
			_e('The selected record(s) deleted successfully!', 'gna-contact-form-7-sms');
			echo '</strong></p></div>';
		}
